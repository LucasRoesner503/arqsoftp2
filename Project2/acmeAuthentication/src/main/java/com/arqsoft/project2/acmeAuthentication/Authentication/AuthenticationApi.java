...
import com.arqsoft.project2.acmeAuthentication.model.UserView;
import com.arqsoft.project2.acmeAuthentication.model.UserViewMapper;
    ...
    public ResponseEntity<UserView> login(@RequestBody @Valid final AuthenticationRequest request) {
        try {
            ...
            final JwtClaimsSet claims = JwtClaimsSet.builder().issuer("example.io").issuedAt(now)
                    .expiresAt(now.plusSeconds(expiry)).subject(String.format("%s,%s", user.getUserId(), user.getUsername()))
